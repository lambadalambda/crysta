...
- Fixed portable itinerary now passes from `GameState::new_game`:276 RLE public-input commands,11,590 actions and per-action restores, all34 invocation keys, both real Town doors/Y400 frozen-bird corridor, off-target throw then two actual hits, shared-sheet E/20 mask7 and21 reset, exact first contact/recoil and distinct polling/COPDF handoff, all forced tour loads,243/244 and stable two-axis final41 control. Discovery geometry/forward simulation helpers were removed; the committed replay uses no planner, state seeding or captured initializer. Source Y370 contact required a fresh Down approach fromY360, not broadening the witness.
- Eleven owned-ROM tests pass, including missing-North-interaction mutation rejection and typed atomic negative probes. Debug/release routes match final snapshot and152-event semantic hashes; fully patched effective rooms and semantic outputs agree after every restore. Generated harness now inherits equivalent all/pedantic/missing-docs policy; harness Clippy and actual workspace temporary-registration Clippy pass. Independent fixed-route correctness/evidence/architecture review approved; latest derived-view assertions rerun green. Report is output-only under ignored `local/pandora-runtime-qualification/report.json`. Keep issue open for parent reproduction, host acceptance and fresh producer pin revalidation; no live enablement or new native timing equivalence is claimed.

## Parent acceptance

Parent reproduced the11-test aggregate and11,590 public actions with per-action
restoration and debug/release agreement; the continuous browser route now passes
through final controlled map41. Fresh producer bridge and all final gates pass;
see [umbrella acceptance](open-pandora-portable-slice.md#parent-acceptance).
The36 frozen-bird differences and narrowly qualified contacts remain explicit;
Y145 is not a derived cutoff, and no wider exploration or native timing is claimed.
