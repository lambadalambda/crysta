...
- The host now enables the authenticated source compiler and core profile9: explicit page/choice commands6..9, sorted event-bit output, source page keys, gate-on-reload and separate exterior data/camera. The source requires real two-option choices and a grant before the first choice, not after all follow-up pages.
- Host red/green coverage now passes all nine first/repeat result combinations, early flags, wrong-action no-ops, movement-only ticks, fatal acknowledgement/choice overflow, reset interruption and the exterior endpoint, with per-action restored continuation. Independent compiler, transport and cross-component reviews found no remaining blockers.

## Accepted result

- Two identical actual-browser1671-step journeys passed20 semantic checkpoints and1683 full canvas checks each, including first acknowledgement granting0026 before choice0, repeat cancel/option1, gate reload, A landing and walking to`(538,815)`.
- Nonblank evidence:44,250 font/choice ink pixels per run; the repeat run additionally retained133,500 visible exterior Ark pixels. Composition checks cover only the admitted layers/actors, not native whole-RGB equality. All original page metadata are checked; entry and first option2/cancel follow-up pages are source-only in this browser route.
- Integrated legacy511/512-canvas and full-house2244/2245-canvas browser routes passed. All336 workspace tests, strict workspace Clippy, native/Wasm builds, controller/verifier tests and safety/tracker gates passed.390px layout had no horizontal overflow and browser errors were empty.
- Source compiler, transport, and final cross-component independent reviews found no remaining blockers. Reproduction, exact semantic ticks and ignored local evidence paths are in[playable-house.md](../../docs/playable-house.md#accepted-talk-and-leave-evidence).
- The live frontend remains a native Rust loopback host plus canvas; the core builds for Wasm but browser Wasm hosting is not implemented. Intro/entry presentation, other NPC conversations/AI, full Crysta, audio and combat remain outside this milestone.
