...
def verify(name,start=1601,grid_frame=None):
    rows=[r for r in csv.DictReader(open(ROOT/name/'frames.csv')) if int(r['frame'])>=start]
    ...
    for prev,r in zip(rows,rows[1:]):
        ...
        try:
            pred=collide(w,int(prev['x']),int(prev['y']),dx,dy)
            assert pred==(int(r['x']),int(r['y'])),(name,r['frame'],pred,r['x'],r['y'])
            qualified.append(int(r['frame']))
        except Unqualified as e:excluded.append((r['frame'],str(e)))
    print(name,'matched',len(qualified),'excluded',len(excluded),excluded[:2])
    if not os.getenv('NAIVE_CADENCE'):
        (ROOT/name/'qualification.json').write_text(json.dumps(dict(start_frame=start, csv_sha256=sha256((ROOT/name/'frames.csv').read_bytes()).hexdigest(), grid_sha256=sha256(w).hexdigest(), matched_position_frames=qualified, rejected_position_frames=excluded),indent=2)+'\n')
