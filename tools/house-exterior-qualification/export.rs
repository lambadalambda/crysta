...
fn main() {
    ...
    for id in [0xa] {
        ...
        assert_eq!((bg.layer().width(), bg.layer().height()), (64, 80));
        let directory = out.join(format!("{id:x}"));
        std::fs::create_dir(&directory).unwrap();
        let mut indices = Vec::new();
        let mut priorities = Vec::new();
        let mut rgb = Vec::new();
        for y in 0..bg.layer().height() * 16 {
            for x in 0..bg.layer().width() * 16 {
        ...
        reports.push(json!({"map":id,"width_cells":bg.layer().width(),"height_cells":bg.layer().height(),"files":files,"sources":sources}));
    }
    std::fs::write(
